...
## 2025-05-23 - [Form Label Accessibility]
**Learning:** Form `<label>` elements in `index.html` must include `for` attributes that correspond exactly to their associated input IDs. This ensures correct click-to-focus behavior and screen-reader accessibility for the inputs. Testing this behavior can be done using Playwright to click the label and check if the input is focused.
**Action:** Always verify that `<label>` tags use the `for` attribute and that it exactly matches the `id` of the target input element.

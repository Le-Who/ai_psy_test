...
## 2024-05-24 - [Empty State CTA Reusable Pattern]
**Learning:** The application's glassmorphism style requires that dynamically injected UI states (like the empty library view) be wrapped in the existing `.card` class. Unstyled text inside a dynamic container breaks the visual hierarchy.
**Action:** When generating isolated UI components or empty states in JavaScript, always wrap them in the standard design system containers (like `.card`) rather than leaving them as floating text, and provide a clear CTA button using the `.btn` class.

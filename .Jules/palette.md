...
## 2026-02-01 - [Label Accessibility]
**Learning:** Native `<label>` elements without `for` attributes fail to associate correctly with their underlying inputs, breaking click-to-focus behavior and diminishing screen-reader accessibility.
**Action:** Always link `<label>` elements to their associated inputs using the `for` attribute and matching `id`s, specifically in pure HTML forms where implicit wrapping might not apply or be possible.

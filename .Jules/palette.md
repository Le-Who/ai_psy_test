...
**Learning:** Missing `for` attributes on `<label>` elements degrades the experience for screen reader users and prevents clicking the label to focus the target input. This makes form interaction less intuitive, particularly on touch devices.
**Action:** Always ensure that `<label>` elements have a `for` attribute that strictly matches the `id` of their corresponding `<input>`, `<select>`, or `<textarea>`.

...
## 2026-02-02 - [Empty States]
**Learning:** Empty states without a clear next action lead to dead ends. Empty states should always use the primary `.card` component for visual consistency and include an actionable CTA (like a `.btn`) to guide the user back into the primary flow.
**Action:** When designing or refactoring an empty state, wrap it in a `.card` and include a button that redirects to the primary feature creation view.

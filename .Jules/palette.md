...
## 2025-05-23 - [Form Label Accessibility and Interaction]
**Learning:** Without explicit `for` attributes on `<label>` elements, clicking the label does not focus the associated input field, and screen readers fail to reliably associate the label text with the input context. This significantly impacts both usability for mouse/touch users and accessibility for assistive technologies.
**Action:** Always verify that every `<label>` is explicitly linked to its input using the `for` attribute that exactly matches the input's `id`.

...
## 2024-04-07 - [Explicit Label Associations]
**Learning:** Implicit label association (wrapping) or missing associations break click-to-focus behaviors and reduce screen reader clarity, particularly for dynamically rendered or custom-styled form groups.
**Action:** Always use explicit `for` attributes on `<label>` elements that perfectly match the `id` of their target input, including complex inputs like `<select>` and `<textarea>`.

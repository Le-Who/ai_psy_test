...
## 2026-03-02 - [DOM Query Scoping in Event Handlers]
**Learning:** Performing multiple full-document traversals (`document.querySelectorAll`) inside frequently called event handlers (like answering a quiz question) adds unnecessary overhead, especially when modifying classes on a large list of similar elements.
**Action:** When querying for elements within a known specific area (like a quiz container), scope the query to that cached container (e.g., `this.ui.quizContainer.querySelectorAll`) and reuse the resulting NodeList instead of querying multiple times.

...
## 2026-03-01 - [Redundant Full Document Traversal]
**Learning:** `document.querySelectorAll` was being used redundantly to find child elements (e.g. `.quiz-opt`), which caused full DOM traversal overhead when a cached container element was already available.
**Action:** Scope `querySelectorAll` to the specific cached container (e.g., `this.ui.quizContainer.querySelectorAll(...)`) and execute state changes within a single iteration loop to avoid redundant query and traversal costs.

...
## 2026-03-05 - [Reactive State Redundant Renders]
**Learning:** The application's reactive state manager (`src/store.js`) triggers all listeners on any assignment, even if the value hasn't actually changed. In a UI with heavy DOM rendering like this app, redundant identical assignments cause expensive and unnecessary DOM rebuilds.
**Action:** Always include a short-circuit check (`if (target[prop] === value) return true;`) in the `set` trap of reactive Proxies to prevent propagation of identical state values.

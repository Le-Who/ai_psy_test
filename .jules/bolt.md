...
## 2024-05-18 - Proxy State Redundant Listener Execution
**Learning:** Using a naive Proxy-based pub/sub for state management triggers all listeners (like expensive UI re-renders) on every assignment (`store.prop = value`), even if the value hasn't actually changed.
**Action:** Implement an early return (`if (target[prop] === value) return true;`) in the Proxy `set` trap to short-circuit redundant updates. Also, remember to add defensive environment checks (`typeof window !== 'undefined'`) when attaching global variables like `window.app = app` in UI components to prevent Node.js test environments from crashing.

...
## 2024-10-25 - [Inefficient Regular Expressions for Simple String Checks]
**Learning:** `Utils.escapeHtml` relied entirely on a `.replace()` with a regex to sanitize strings. Even if strings don't contain any characters that need escaping, regex matching still incurs allocation overhead and state mutation. This was problematic as `escapeHtml` is called frequently in hot UI rendering loops.
**Action:** Always add a fast path using `.indexOf()` checks for common strings that don't contain special characters before falling back to regex replacements. `indexOf` is heavily optimized in V8/SpiderMonkey and avoids regex allocation overhead completely.

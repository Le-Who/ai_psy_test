...
## 2026-05-27 - [DOM Query Optimization in Event Handlers]
**Learning:** Redundant `document.querySelectorAll` calls across the entire document during event handlers like `handleQuizAnswer` are an unnecessary performance hit.
**Action:** Always scope DOM queries to specific parent containers (e.g., `this.ui.quizContainer`) or reuse previously queried element collections to minimize DOM traversal.

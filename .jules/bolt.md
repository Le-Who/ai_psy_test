...
## 2026-03-01 - [Inefficient Regex Backtracking for Markdown Parsing]
**Learning:** `safeParseJSON` used a greedy regex `text.match(/```json([\s\S]*?)```/)` which scales poorly on large inputs because of regex backtracking, causing potential O(N^2) execution time.
**Action:** Replace text-bound extractions with exact-match string functions (`indexOf`/`substring`) instead of regex patterns to keep execution time O(N).

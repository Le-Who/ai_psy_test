...
## 2026-03-15 - [Mocking DocumentFragment in VM Tests]
**Learning:** When testing DOM manipulations with custom mocks in `vm`, simple `appendChild` mocks may not handle `DocumentFragment` correctly (flattening children).
**Action:** Ensure mock `appendChild` implementation checks for `nodeType === 11` and spreads `children` to simulate real DOM behavior for verification scripts.

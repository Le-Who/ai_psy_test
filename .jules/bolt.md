...
## 2026-05-29 - [Redundant DOM Writes in Reactive View Setters]
**Learning:** The reactive state manager frequently calls `setView` on sub-state updates (like incrementing `step` during test progress). Without an equality check, this causes redundant full-DOM traversal and style re-calculations, slowing down test progression.
**Action:** Always cache the current UI state globally (e.g., `this._currentView`) and early-return if the requested state matches the current one to prevent unnecessary O(N) DOM writes.

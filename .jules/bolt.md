...
## 2024-05-26 - [Redundant DOM Traversals in Event Handlers]
**Learning:** Performing multiple full-document `querySelectorAll` calls for the same elements within a single event handler execution wastes CPU cycles on redundant traversals.
**Action:** Cache the queried `NodeList` into a variable and reuse it. Furthermore, scope the query to the specific cached UI container (e.g., `this.ui.quizContainer`) with a fallback to `document` to limit the DOM traversal depth.

...
## 2026-03-02 - [Inefficient Regex Backtracking and Switch Execution]
**Learning:** `Utils.escapeHtml` is used heavily when rendering HTML (like testing and results views) and is called repeatedly in loops. Always calling `.replace()` with a global regex on plain text without special characters forces the JS engine to scan the entire string to build a replacement map and execute a callback function even when no changes are needed.
**Action:** Implement a regex fast-path using `.test()` without the `/g` flag. If no HTML entities exist in the text, immediately return the original string, bypassing the slower global replace operation and improving execution speed.

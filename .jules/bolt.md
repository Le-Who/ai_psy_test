...
## 2026-04-18 - [Redundant Document-Wide DOM Queries]
**Learning:** In vanilla JS event handlers like `handleQuizAnswer`, making multiple full-document traversals (`document.querySelectorAll`) for the same elements is slow and inefficient.
**Action:** Cache the result of `querySelectorAll` and scope the query to a specific container (`this.ui.quizContainer || document`) rather than the entire `document` to optimize DOM traversal operations.

...
## 2024-10-24 - [Inefficient String Escaping in Hot Paths]
**Learning:** The `escapeHtml` utility used multiple chained `.replace()` calls or regex matching, causing redundant string traversals. This utility is heavily used in rendering loops (e.g., `renderLibraryHTML`), making it a hidden CPU sink.
**Action:** Use a single character code loop with an initial `indexOf` short-circuit for safe strings. This ensures O(n) complexity and avoids the overhead of regex engine setup and switch-like mappings.

...
## 2026-03-17 - [Redundant Pub/Sub Emissions]
**Learning:** A custom reactive state system based on `Proxy` can emit thousands of redundant DOM update triggers if it blindly calls listeners whenever a property is set to an identical value.
**Action:** Always short-circuit the `set` trap in Proxy-based state managers (`if (target[prop] === value) return true;`) to prevent unnecessary downstream rendering overhead.

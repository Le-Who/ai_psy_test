...
## 2026-03-05 - [Full Re-Renders on Single Item Delete]
**Learning:** Calling `.innerHTML` to re-render the entire library list on every individual item deletion forces the browser to discard the entire DOM tree, re-parse the massive generated HTML string, and repaint it. This is highly inefficient.
**Action:** Always use targeted `.remove()` directly on the `HTMLElement` representing the item when deleting, and only fall back to a full re-render when the state becomes empty or the element can't be reliably found.

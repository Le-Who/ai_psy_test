...
## 2024-10-25 - [Inefficient escapeHtml in Hot Loops]
**Learning:** `Utils.escapeHtml` is called frequently in hot paths like rendering loops. Invoking `.replace()` on strings that don't need escaping adds measurable overhead.
**Action:** Add an early fast path return in `escapeHtml` using a series of `indexOf` checks to quickly identify strings that lack HTML entities, returning them immediately without incurring regex overhead.

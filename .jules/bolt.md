...
## 2024-11-20 - [Optimizing Sanitization via Early Return]
**Learning:** Utilities like `escapeHtml` that use string replacements run on nearly every UI update. Bypassing the `.replace` machinery entirely when no special characters are present provides a significant CPU win in hot paths.
**Action:** Always implement a non-global regex `.test()` check before executing `.replace()` on large or frequent strings to avoid unnecessary string allocations.

...
## 2026-05-12 - [Inefficient Regex Backtracking for Markdown Code Blocks]
**Learning:** `safeParseJSON` used a regex `text.match(/```json([\s\S]*?)```/)` to extract JSON from markdown responses. The regex engine's capture groups and potential backtracking overhead made this operation up to 20x slower than simple string search on large text blobs.
**Action:** Use `indexOf` to find the boundaries of known block markers (e.g. ` ```json ` and ` ``` `) to extract content significantly faster with O(N) complexity and no backtracking.

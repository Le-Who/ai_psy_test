...
## 2026-03-01 - [Regex fast-paths and the /g flag]
**Learning:** Using a regex test (like `!/[&<>"']/.test(string)`) before a string replace can be an effective fast-path to avoid slow replace allocations for clean strings, especially in hot rendering loops. However, if the test regex uses the global `/g` flag, it will mutate the `.lastIndex` property across consecutive calls, causing intermittent matching failures.
**Action:** Always ensure that test regular expressions used for fast-paths do not have the `/g` flag.

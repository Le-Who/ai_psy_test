...
## 2026-02-27 - [Desynchronization Risk in O(1) DOM Removals]
**Learning:** Optimizing a list deletion by directly removing the DOM node (`element.remove()`) instead of triggering a full state-based re-render is highly performant (O(1) vs O(N)). However, doing this unconditionally without checking if the specific target element exists can lead to a silent failure where the UI becomes completely out of sync with the underlying data state.
**Action:** When implementing O(1) direct DOM manipulations as an optimization over a full re-render, always include a robust fallback path (e.g., triggering the full re-render) if the targeted DOM node cannot be successfully located and removed.

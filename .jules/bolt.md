...
## 2024-03-21 - [DOM Query Optimization in Event Handlers]
**Learning:** Calling `document.querySelectorAll` multiple times for the same static or semi-static node list within a single function (like `handleQuizAnswer`) adds unnecessary overhead, especially when the first query can be cached and reused.
**Action:** Always cache the result of `document.querySelectorAll` in a variable if the same elements need to be iterated over or accessed again within the same function scope.

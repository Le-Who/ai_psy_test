...
## 2026-05-24 - [Optimize redundant document-wide DOM queries]
**Learning:** Repeatedly calling `document.querySelectorAll` within a single event handler execution loop adds unnecessary overhead by searching the entire DOM multiple times.
**Action:** When working with dynamic elements inside a specific container, query from the cached parent container (e.g., `this.ui.quizContainer`) and reuse the returned NodeList instead of querying the global `document` multiple times.

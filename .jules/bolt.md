...
**Learning:** Frequent `document.querySelectorAll` calls inside user interaction event handlers (`handleQuizAnswer`) introduce measurable overhead when the query covers the entire document.
**Action:** Cache the resulting NodeList when applying multiple operations to the same queried elements. In addition, scope DOM queries to the cached parent container elements (with a safe `document` fallback for detached execution contexts) instead of searching the entire DOM tree.

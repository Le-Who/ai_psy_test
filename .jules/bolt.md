...
## 2024-05-26 - [DOM Query Scoping with Safe Fallback]
**Learning:** Frequent full-document DOM queries (like `document.querySelectorAll`) inside event handlers can be optimized by scoping to a cached container element. However, using `this.ui?.container` blindly can crash in detached callbacks or ES modules strict mode where `this` is undefined.
**Action:** Use a robust fallback check like `(this && this.ui && this.ui.quizContainer) || document` when substituting document-level queries with cached scoped queries inside event handlers to prevent `TypeError` crashes while still reaping performance benefits.

...
## 2026-03-02 - [Direct DOM Mutation over Full Re-renders]
**Learning:** Full library re-renders using `innerHTML` on deletion are O(N) and slow down the UI, especially for large lists.
**Action:** When a user deletes an item, manually remove the `.card` element using `.remove()` instead of immediately rebuilding and replacing the entire list's HTML string. Fall back to full re-render only when necessary (like emptying the library).

## 2026-03-02 - [Proxy State Redundant Execution]
**Learning:** Reactive state implementations without value equality checks execute all listeners even when the property is assigned the same value it already holds.
**Action:** Always add `if (target[prop] === value) return true;` short-circuiting in Proxy set traps to prevent redundant listener execution for primitive values.

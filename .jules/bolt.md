...
## 2026-03-02 - [Regex Optimization for Fast-Path]
**Learning:** Using `RegExp.replace()` unconditionally creates string allocation and callback execution overhead.
**Action:** When performing string sanitization (like `escapeHtml`), use `RegExp.test()` first to verify if special characters exist before calling `replace()`. This provides an O(N) fast-path that is significantly faster for safe strings.

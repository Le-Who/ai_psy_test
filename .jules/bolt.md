...
## 2026-03-02 - [Redundant State Rendering]
**Learning:** The reactive state manager (`src/store.js`) used a Proxy to intercept top-level property assignments but lacked a short-circuit check (`if (target[prop] === value) return true;`). This caused redundant listener executions and DOM re-renders whenever a property was reassigned to its existing value.
**Action:** Always include a short-circuit check in Proxy `set` handlers or state setter functions to prevent unnecessary downstream computations for unchanged values.
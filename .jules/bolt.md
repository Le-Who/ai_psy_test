...
## 2024-05-26 - [Fast-path optimization for String replacements]
**Learning:** Checking for the presence of target characters using a non-global regex `RegExp.test()` before executing `.replace()` provides a massive speedup (3x faster) for clean strings, bypassing expensive allocations. Using a global `/g` flag on the test regex would cause intermittent failures due to stateful `.lastIndex` mutations.
**Action:** When writing utilities that conditionally escape or replace text (like `escapeHtml`), always add a fast-path test using a non-global regex before doing the actual `.replace()`.

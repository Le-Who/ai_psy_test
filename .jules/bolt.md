...
## 2024-05-25 - [Regex Replace Callback Setup Overhead]
**Learning:** `String.prototype.replace(regex, callback)` incurs significant hidden overhead just to setup the function invocation, even when no matches are found, causing 100%+ slower execution in hot-paths rendering large lists compared to a simple regex `.test()`.
**Action:** When using `.replace()` with a callback in hot paths (like escaping HTML during list rendering), always add a fast-path early return using `RegExp.test()` without the `/g` flag to skip processing strings that don't need replacement.

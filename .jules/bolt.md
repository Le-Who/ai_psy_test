...
## 2024-05-26 - [Redundant DOM Queries in Event Handlers]
**Learning:** Calling `document.querySelectorAll` multiple times within event handlers like `handleQuizAnswer` adds unnecessary overhead, especially when the elements are scoped to a specific container.
**Action:** Query the elements once and reuse the NodeList. Scope queries to a specific container (e.g., `this.ui.quizContainer`) instead of `document` to reduce the DOM search space. Always use a safe fallback `(this && this.ui && this.ui.quizContainer) || document` to prevent `TypeError` crashes in detached contexts.

...
## 2024-10-25 - [Inefficient Regex Replacement on Clean Strings]
**Learning:** `Utils.escapeHtml` was using `String.prototype.replace()` with a regex on every string passed to it, even those without HTML special characters. This is computationally expensive, especially since `escapeHtml` is called repeatedly inside large string rendering loops (e.g. `renderLibraryHTML`, list items).
**Action:** Always implement a fast-path pre-check using `RegExp.test()` without the `/g` flag before performing regex replacements on potentially clean strings. This avoids the overhead of `.replace()` for the majority of safe strings.

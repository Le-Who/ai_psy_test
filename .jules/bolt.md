...
## 2024-05-26 - [Safe Contextual DOM Scoping]
**Learning:** When scoping DOM queries to a cached container in event handlers (e.g., `this.ui.quizContainer`), using optional chaining (`this.ui?.something`) can cause `TypeError` crashes if `this` evaluates to `undefined` (e.g., in strict mode ES modules or detached callbacks).
**Action:** Use a safe fallback explicitly checking `this` before accessing its properties: `(this && this.ui && this.ui.quizContainer) || document` to ensure robustness.

...
## 2026-03-01 - [Redundant Full-Document Queries in Event Handlers]
**Learning:** In vanilla JS event handlers, performing multiple full-document queries (`document.querySelectorAll`) for the same elements adds unnecessary overhead, and blindly querying `.children` on parent containers is risky if the DOM structure changes.
**Action:** Cache the result of `querySelectorAll` when performing multiple operations on the same elements, and scope the query to a specific container (e.g., `(this && this.ui && this.ui.quizContainer) || document`) to avoid full-document traversals while maintaining a safe fallback.

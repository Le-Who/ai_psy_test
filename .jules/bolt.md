...
## 2024-07-10 - Avoid greedy regex for LLM text blocks
**Learning:** Parsing large LLM responses to extract code blocks (like JSON) using lazy regex (e.g. `/[\s\S]*?/`) is extremely slow and can lead to performance bottlenecks.
**Action:** Replace regex parsing with `indexOf` and `substring` to ensure efficient O(N) extraction. Use this approach whenever parsing structured text out of large, unstructured text blocks.

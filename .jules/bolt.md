...
## 2026-03-01 - [Redundant Full-Document Traversals in Event Handlers]
**Learning:** Calling `document.querySelectorAll` multiple times for the same elements within an event handler (like `handleQuizAnswer`) adds measurable overhead, especially when interacting with a deeply nested DOM.
**Action:** Cache the resulting `NodeList` if the same elements are processed multiple times. Furthermore, scope the DOM query to a specific cached container rather than the full `document` to limit traversal depth, providing a safe fallback to prevent `undefined` references (e.g., `const container = (this && this.ui && this.ui.quizContainer) || document;`).

...
## 2026-03-02 - [Unscoped DOM Queries in Handlers]
**Learning:** Frequent unscoped `document.querySelectorAll` calls inside event handlers (like `handleQuizAnswer`) scan the entire DOM tree unnecessarily, introducing overhead that can cause stuttering during rapid UI interactions.
**Action:** When updating a group of elements within a specific component, scope the query to that component's cached container (e.g., `this.ui.quizContainer.querySelectorAll`) rather than the entire `document`.

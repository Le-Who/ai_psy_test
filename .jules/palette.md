...
## 2026-05-26 - [Form Label Accessibility]
**Learning:** By default, several form `<label>` elements in the setup view lacked explicit `for` attributes, relying solely on visual proximity to their inputs. This prevents screen readers from properly associating the label text with the input field, and prevents clicking the label from focusing the input.
**Action:** Always ensure `<label>` elements have an explicit `for` attribute that precisely matches their corresponding input's `id` to properly connect them for standard HTML5 accessibility and click-focus behavior.

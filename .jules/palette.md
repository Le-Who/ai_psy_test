...
## 2026-02-01 - [Label click-to-focus]
**Learning:** Form `<label>` elements without `for` attributes severely degrade both keyboard navigation and mobile usability, as users cannot click the label text to focus the associated input or trigger hidden controls (like select boxes).
**Action:** Always link `<label>` tags to their specific input fields using the `for` attribute matching the input's `id`.

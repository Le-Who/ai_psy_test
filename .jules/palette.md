...
## 2024-05-25 - [Form Labels Accessibility]
**Learning:** Inputs inside complex forms that rely solely on proximity to labels (`<label>Title</label> <input>`) are not programmatically linked. This causes screen readers to announce inputs without context and breaks the native click-to-focus behavior.
**Action:** Always include the `for` attribute on `<label>` elements matching the exact `id` of the corresponding input field to ensure programmatic association and improve UX (especially for touch targets).

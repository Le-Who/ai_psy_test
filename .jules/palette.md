...
## 2025-05-23 - [Form Label Accessibility & Click Targets]
**Learning:** Screen readers and keyboard navigation rely heavily on explicitly linking `<label>` elements to their form controls via the `for` attribute and the input's `id`. Without this, screen readers may announce inputs incorrectly, and the clickable target area for users to focus the input is reduced to just the input box itself.
**Action:** Always include the `for` attribute on `<label>` elements matching the exact `id` of the corresponding form input, especially in forms with conditional visibility (like the Quiz/Psy modes).

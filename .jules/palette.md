...
## 2026-04-10 - [Explicit Label Associations for Forms]
**Learning:** Custom forms with nested elements (like `<span>` asterisks inside a `<label>`) require explicit `for` attributes mapping exactly to the target input's ID. Implicit wrapping can fail click-to-focus behavior and create poor hit areas, reducing accessibility for screen readers and usability for pointer events.
**Action:** Always link `<label>` elements explicitly using `for` attributes rather than relying solely on DOM structure. When testing with Playwright, test explicit labels by clicking the precise `label[for='ID']` locator.

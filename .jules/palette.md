...
## 2026-05-20 - [Explicit Label Associations]
**Learning:** Nested elements inside labels (like asterisks) can break strict text matching for screen readers and testing frameworks. Implicit association is often not enough.
**Action:** Always use the `for` attribute on `<label>` elements and link it precisely to the corresponding input's `id`.

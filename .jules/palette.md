...
**Learning:** Native `<label>` elements without a `for` attribute (or wrapping the input) are ignored by screen readers and don't provide the click-to-focus behavior expected by users, especially on mobile devices where tap targets need to be larger. Nested visual elements like an asterisk span (`<span>*</span>`) can further complicate screen reader parsing if the `for` attribute isn't explicitly defined.
**Action:** Always link `<label>` elements to their associated inputs using the `for` attribute and the exact input `id`.

...
**Learning:** Nested elements inside `<label>` tags (like `<span>` asterisks for required fields) can cause automated accessibility checks and strict text matching to fail if the label isn't explicitly linked to its input. Relying on implicit wrapping isn't always sufficient.
**Action:** Always explicitly link form `<label>` elements to their associated inputs using the `for` attribute and the input's `id`, regardless of the DOM structure.

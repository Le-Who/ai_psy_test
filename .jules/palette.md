...
## 2026-05-25 - [Accessible Form Labels]
**Learning:** Implicit label association (wrapping input with label) or missing associations can cause accessibility issues with screen readers and focus clicks. For standard HTML5 accessibility, always ensure `<label>` elements have an explicit `for` attribute that precisely matches their corresponding input's `id`.
**Action:** Use explicit `for` attributes on all `<label>` elements to properly connect them for screen readers and click-focus associations.

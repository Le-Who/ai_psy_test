...
**Learning:** Nested elements (like `<span>` asterisks) inside form labels can cause strict text matching to fail in testing and can sometimes affect screen readers.
**Action:** Always link `<label>` tags explicitly to their inputs using the `for` attribute and the input's `id`.

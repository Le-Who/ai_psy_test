...
**Learning:** Implicit or missing `<label for="...">` associations prevent click-to-focus behavior and degrade screen reader experience. Strict text matching for nested elements (like required asterisks) can cause tests to fail.
**Action:** Always link `<label>` elements to their corresponding inputs using explicit `for`/`id` attributes, and verify via explicit locators (`label[for='id']`) rather than generalized text matches.

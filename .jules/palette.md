...
## 2026-05-08 - [Explicit Form Labels]
**Learning:** Implicit label wrapping or relying purely on structural proximity can cause issues for screen readers, especially when labels contain nested HTML elements like `<span>` (e.g., for required asterisks). This also causes strict text-matching locators in Playwright to fail.
**Action:** Always add an explicit `for` attribute to `<label>` elements that precisely matches the corresponding input's `id` to ensure robust accessibility, click-focus associations, and reliable test automation.

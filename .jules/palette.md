...
## 2026-04-26 - [Form Label Accessibility]
**Learning:** Implicit label wrapping (e.g. `<label>Name <input></label>`) can be problematic when the label contains nested elements like asterisks inside a `<span>`, causing some screen readers or strict focus matching to fail.
**Action:** Always use explicit `for` attributes linked to the target input's `id` to ensure robust accessibility, screen reader announcements, and larger clickable areas for focus management.

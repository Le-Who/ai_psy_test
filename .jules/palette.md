...
## 2025-07-02 - [Form Label Accessibility & Playwright Verification]
**Learning:** Adding `for` attributes to form labels improves screen reader accessibility and mobile usability, but Playwright tests can struggle to click them natively if they're hidden by CSS or animations (like `#audienceGroup`).
**Action:** Use `page.evaluate("document.querySelector('label').click()")` to reliably click visually-complex labels during testing, and assert `expect(input).to_be_focused()` to ensure proper `for` attribute linking.

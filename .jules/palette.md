...
## 2024-05-27 - [Label Association]
**Learning:** Standard HTML5 accessibility relies on `<label>` elements having an explicit `for` attribute that matches their corresponding input's `id`. Relying solely on DOM proximity or wrapping is insufficient for screen readers and does not enable click-focus.
**Action:** Always ensure all `<label>` tags use `for` attributes correctly linked to input `id`s when creating or modifying forms.

...
## 2025-05-23 - [Label and Input Association]
**Learning:** In standard HTML5, `<label>` elements must either wrap their target inputs or have an explicit `for` attribute matching the input's `id`. Relying purely on visual proximity breaks screen reader announcement and prevents click-to-focus behavior.
**Action:** Always verify that `<label>` elements have an explicit `for` attribute mapping precisely to the target input's `id`, particularly when the label and input are separated in the DOM structure.

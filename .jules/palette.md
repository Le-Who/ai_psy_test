...
## 2026-05-25 - [Explicit Label Associations]
**Learning:** Implicit label associations or text-matched locators often fail when labels contain nested elements (like `<span>*</span>` for required fields). Explicitly linking labels to inputs using `for` and `id` ensures robust screen reader support and increases the click target area for focusing inputs.
**Action:** Always use explicit `for` attributes on `<label>` elements matching the target input's `id`, rather than relying solely on visual proximity or implicit wrapping.

...
## 2025-06-15 - [Form Label Accessibility]
**Learning:** Form `<label>` elements missing explicit `for` attributes cause multiple issues: screen readers fail to associate the label with its input, click-to-focus behavior breaks for mouse users, and strict Playwright locators using `.getByLabel()` fail, especially if nested elements (like `<span>` asterisks) are present inside the label text.
**Action:** Always include a `for` attribute on `<label>` tags that corresponds exactly to their associated input `id`.

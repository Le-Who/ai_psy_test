...
**Learning:** Labels containing nested elements (like `<span class="text-danger">*</span>` for required fields) can prevent screen readers and Playwright strict text matching from functioning correctly if they rely solely on text content. Additionally, users clicking the label will not focus the input without an explicit ID link.
**Action:** Always link `<label>` to its corresponding `<input>` or `<textarea>` using the `for` attribute and an explicit `id`, regardless of DOM nesting.

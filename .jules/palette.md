...
**Learning:** Implicit label wrapping or visual proximity is insufficient when labels contain nested elements (like `<span class="text-danger">*</span>`). This breaks strict text matching in automated tests (Playwright) and can confuse screen readers.
**Action:** Always use explicit `for` attributes on `<label>` elements that perfectly match the `id` of the target input, ensuring rock-solid semantic association and reliable test selectors.

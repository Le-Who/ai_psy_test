...
## 2025-05-23 - [Form Accessibility & Micro-UX]
**Learning:** Labels missing explicit links (`for` pointing to an `id`) prevent users from clicking the text to focus the field, and screen readers fail to associate the label text with the input context.
**Action:** Form `<label>` elements must always include `for` attributes that correspond exactly to their associated input IDs to ensure correct click-to-focus behavior and screen-reader accessibility.

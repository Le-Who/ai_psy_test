...
**Learning:** Standard HTML5 `<label>` elements without an explicit `for` attribute are not programmatically linked to their corresponding inputs if the input is not nested inside the label. This is especially problematic when labels contain nested `<span>` elements (like required asterisks), causing screen readers and click-focus associations to fail.

...
## 2025-05-23 - [Form Accessibility: Explicit Labels]
**Learning:** Implicit label association (wrapping inputs or relying on adjacent placement) is brittle, especially when `<label>` tags contain nested elements like `<span class="text-danger">*</span>` for required indicators. This can cause strict text matching in automated tests (like Playwright) to fail and can reduce screen reader reliability.
**Action:** Always use explicit label association by setting the `for` attribute on `<label>` elements to match the exact `id` of the target input, ensuring robust click-to-focus behavior and testability.

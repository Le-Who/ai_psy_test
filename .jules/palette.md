...
## 2024-06-17 - Added missing 'for' attributes to form labels
**Learning:** Implicit label linking (nesting input inside label) or missing label associations break accessibility for screen readers and prevent users from clicking the label text to focus the target input.
**Action:** Always ensure form `<label>` elements explicitly link to their corresponding interactive elements (inputs, textareas, selects) using the `for` attribute referencing the element's `id`.

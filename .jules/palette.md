...
**Learning:** Form labels must always include the `for` attribute referencing their corresponding input `id` (e.g., `<label for="themeInput">`). This ensures screen readers announce the input's purpose and enables click-to-focus behavior, especially when the label text contains nested elements like `<span>` asterisks which can disrupt strict text matching.
**Action:** Always link `<label>` and `<input>` elements directly using `for` and `id` attributes across all form designs.

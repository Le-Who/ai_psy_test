...
## 2026-07-16 - [Form Label Associations]
**Learning:** Adding the `for` attribute to `<label>` tags is critical for accessibility. It explicitly links the label text to the form input for screen readers and creates a larger clickable area for the input field, improving usability for all users, especially on mobile devices or for users with motor difficulties. We discovered a pattern in this app's forms where labels were visual but not programmatically associated with inputs.
**Action:** Always link a `<label>` to its corresponding `<input>` or `<select>` via the `for` attribute and input `id`.

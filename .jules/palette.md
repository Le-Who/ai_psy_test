...
## 2025-05-23 - [Form Accessibility via Explicit Labels]
**Learning:** Even simple forms suffer accessibility issues if labels lack explicit `for` attributes linking them to input `id`s. Implied wrapping (`<label><input></label>`) isn't used here, so explicit IDs are required for click-to-focus and screen reader announcements.
**Action:** Always link `<label>` elements to inputs using the `for` attribute and the exact input `id`.

...
**Learning:** A standard HTML `<label>` missing the `for` attribute completely fails to connect to its target input in screen readers. It breaks focus associations and makes the entire form inaccessible.
**Action:** Always add an explicit `for` attribute mapping to the target input's `id`, even for simple forms. Never rely solely on visual proximity.

...
export const app = {
	...
	async createShareLink(btnEl = null) {
		// 🛡️ Sentinel: Removed hardcoded TINYTOKEN and implement BYOK via localStorage
		let tinyToken = localStorage.getItem("tiny_api_token");
		if (!tinyToken) {
			tinyToken = prompt(
			...
			localStorage.setItem("tiny_api_token", tinyToken);
		}
	...
	async saveTest(btnEl = null) {
		...
		try {
			// 🛡️ Sentinel: Read token from localStorage, gracefully skip if not present

...
export const app = {
	...
	async createShareLink(btnEl = null) {
		let tinyToken = localStorage.getItem("tiny_url_token");
		if (!tinyToken) {
			tinyToken = prompt("Введите TinyURL Token для создания короткой ссылки:");
			if (!tinyToken) return alert("Нужен TinyURL Token!");
			localStorage.setItem("tiny_url_token", tinyToken);
		}
	...
	async saveTest(btnEl = null) {
		...
		try {
			const tinyToken = localStorage.getItem("tiny_url_token");

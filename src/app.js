...
export const app = {
	...
	async createShareLink(btnEl = null) {
		let tinyToken = localStorage.getItem("tinyurl_token");
		if (!tinyToken) {
			tinyToken = prompt(
				"Введите TinyURL API Token для создания коротких ссылок:",
			);
			if (tinyToken) {
				localStorage.setItem("tinyurl_token", tinyToken.trim());
			} else {
				this.showToast("Создание короткой ссылки отменено.");
				return;
			}
		}
		...
		try {
			...
			const response = await fetch("https://api.tinyurl.com/create", {
				method: "POST",
				headers: {
					Authorization: `Bearer ${tinyToken.trim()}`,
	...
	async saveTest(btnEl = null) {
		...
		try {
			if (typeof LZString !== "undefined") {
				let tinyToken = localStorage.getItem("tinyurl_token");
				// Optionally prompt on save if token is not there
				// For save we might not want to block, just skip short URL generation if no token

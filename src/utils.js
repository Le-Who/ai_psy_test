...
export const Utils = {
	...
	escapeHtml: (unsafe) => {
		...
		// Using indexOf is significantly faster (O(N) search) than invoking
		// the regex engine and its callback for strings that don't need escaping,
		// which drastically improves performance in rendering loops.
		if (
			unsafe.indexOf("&") === -1 &&
			unsafe.indexOf("<") === -1 &&
			unsafe.indexOf(">") === -1 &&
